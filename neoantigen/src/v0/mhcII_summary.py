...
def get_combine(ptID, input_dir, outDir, length, ale):
	'''
	in_smm_tocheck = input_dir + 'classII_' + ptID + '_smm_HLA-DRB30101_length_'+ str(length)+'.log'
	in_nn_tocheck = input_dir + 'classII_' + ptID + '_nn_HLA-DRB30101_length_' + str(length)+'.log'
	in_net_tocheck = input_dir + 'classII_' + ptID + '_NetMHCIIpan_HLA-DRB30101_length_' + str(length)+'.log'
	in_comblib_tocheck = input_dir+ 'classII_' + ptID + '_comblib_HLA-DRB30101_length_' + str(length)+'.log'
	in_sturniolo_tocheck = input_dir+ 'classII_' + ptID + '_sturniolo_HLA-DRB30101_length_' + str(length)+'.log'
	'''
	...
	in_net_tocheck = input_dir + '/' + ptID+ '_IEDB_NetMHCIIpan_' + str (ale) + '_length_' + str (length) + '.log'
	...
	if os.path.isfile (in_net_tocheck):
		...
		try:
			...
			in_net = pd.DataFrame (
				columns=['allele', 'seq_num', 'start', 'end', 'length', 'netmhciipan_core_peptide', 'peptide',
						 'netmhciipan_ic50', 'netmhciipan_percentile_rank', 'netmhciipan_adjusted_rank'])
			df_net_ic50 = in_net
		else:
			df_net_ic50 = df_net.loc[df_net['ic50'] < 500]
			df_net_ic50.columns = ['allele', 'seq_num', 'start', 'end', 'length', 'netmhciipan_core_peptide', 'peptide',
							   'netmhciipan_ic50', 'netmhciipan_percentile_rank', 'netmhciipan_adjusted_rank']
	else:
		in_net = pd.DataFrame (
			columns=['allele', 'seq_num', 'start', 'end', 'length', 'netmhciipan_core_peptide', 'peptide',
	...
	### add all callers into a new column
		df_join['Methods'] = df_join[['NetMHCIIpan', 'comblib', 'SMM_align', 'NN_align', 'sturniolo']].apply (
	...
	# Get a series containing maximum value of each row
		df_join = df_join[['allele', 'seq_num', 'start', 'end', 'length', 'peptide', 'Methods', 'Nmethods', 'comblib_core_peptide', 'comblib_ic50', 'comblib_percentile_rank', 'comblib_adjusted_rank', 'nn_core_peptide', 'nn_ic50','nn_percentile_rank', 'nn_adjusted_rank', 'smm_core_peptide', 'smm_ic50', 'smm_percentile_rank','smm_adjusted_rank', 'sturniolo_core_peptide', 'sturniolo_score', 'sturniolo_percentile_rank','sturniolo_adjusted_rank', 'netmhciipan_core_peptide', 'netmhciipan_ic50', 'netmhciipan_percentile_rank','netmhciipan_adjusted_rank']]
...
def main():
	...
	lns = pts.readlines ()
	input_classII = input_dir + '/output/classII/IEDB/'

	for i in range ((int) (start)-1, (int) (end)):
		#print (lns[i])
		tmp = lns[i].split ('\n')
		ptID = tmp[0]  # tumor patient ID
		hla_file = input_dir + '/input/hlalist/classII/' + ptID + '.hla.IEDBII.txt'
		hlas = open (hla_file)
